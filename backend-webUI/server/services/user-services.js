 ...
 */
const getKeyFromBody = function(body){
...
const authUser = async function(UserInfo){ 
    ...
    if(validator.isEmail( key )){
        ...
        [err, user] = await to(User.findOne({where:{email:key}}));
        //If email not found in Db
        if(err) ThrowErr(err.message);
    } else if( validator.isMobilePhone(unique_key, 'any')){ 
        auth_info.method='phone';
        [err, user] = await to(User.findOne({where:{phone:key }}));
        //If mobile phone not found in Db
        if(err) ThrowErr(err.message);
    } else{
